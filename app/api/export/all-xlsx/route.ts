/**
 * API route for exporting all data to XLSX format
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${fastApiUrl}/api/export/all-xlsx`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      { error: "Failed to export all XLSX" },
      { status: 500 }
    );
  }
}