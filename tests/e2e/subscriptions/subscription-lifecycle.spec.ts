...
test.describe("Subscription Lifecycle", () => {
  ...
  test.describe("Automatic Renewal", () => {
    test("should process successful renewal", async ({
      page,
      auth,
      payment,
      database,
    ...
    test("should handle renewal payment failure", async ({
      page,
      auth,
      payment,
      database,
      ...
      await expect(
        page.locator('[data-cy="payment-failed-banner"]'),
      ).toBeVisible();
      ...
      await expect(
        page.locator('[data-cy="update-payment-method"]'),
      ).toBeVisible();
    });
    ...
    test("should enter grace period after payment failure", async ({
      ...
      await expect(
        page.locator('[data-cy="grace-period-warning"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-cy="grace-period-warning"]'),
      ).toContainText(/days remaining/i);
  ...
  test.describe("Credit Management", () => {
    test("should track credit usage accurately", async ({
      page,
      auth,
      payment,
      database,
    ...
    test("should show credit warning at threshold", async ({
      page,
      auth,
      payment,
      database,
      ...
      await expect(page.locator('[data-cy="credit-warning"]')).toContainText(
        /5 credits remaining/i,
      );
    });

    test("should prevent actions when out of credits", async ({
      page,
      auth,
      payment,
      database,
      ...
      await expect(
        page.locator('[data-cy="out-of-credits-modal"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-cy="upgrade-to-business"]'),
      ).toBeVisible();
    });

    test("should reset credits on renewal date", async ({
      page,
      auth,
      payment,
      database,
  ...
  test.describe("Mid-cycle Changes", () => {
    test("should handle immediate upgrade with proration", async ({
      page,
      auth,
      payment,
      database,
    ...
    test("should schedule downgrade for end of period", async ({
      ...
      const renewalDate = await page
        .locator('[data-cy="scheduled-change-date"]')
        .textContent();
  ...
  test.describe("Pause and Resume", () => {
    test("should pause subscription at end of period", async ({
      page,
      auth,
      payment,
    }) => {
      ...
      await expect(
        page.locator('[data-cy="subscription-status"]'),
      ).toContainText("Active");

      // Can cancel pause
      await page.locator('[data-cy="cancel-pause"]').click();
      await expect(
        page.locator('[data-cy="pause-scheduled"]'),
      ).not.toBeVisible();
    });

    test("should maintain read-only access when paused", async ({
      page,
      auth,
      payment,
      database,
      ...
      await page.locator('[data-cy="upload-button"]').click();
      await expect(
        page.locator('[data-cy="subscription-paused-modal"]'),
      ).toBeVisible();
    });
  ...
  test.describe("Billing Notifications", () => {
    test("should send renewal reminder emails", async ({
      page,
      auth,
      payment,
      database,
    ...
    test("should notify on payment failure", async ({
      page,
      auth,
      payment,
      database,
      ...
      await page.reload();
      await expect(
        page.locator('[data-cy="notification-badge"]'),
      ).toBeVisible();

      // Click to view
      await page.locator('[data-cy="notifications-button"]').click();
      await expect(
        page.locator('[data-cy="notification-item"]').first(),
      ).toContainText("Payment failed");
    });

    test("should show card expiration warning", async ({
      page,
      auth,
      payment,
      database,
      ...
      await expect(
        page.locator('[data-cy="card-expiring-warning"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-cy="update-card-button"]'),
      ).toBeVisible();
    });
  });

  test.describe("Grace Period Management", () => {
    test("should allow payment retry during grace period", async ({
      page,
      auth,
      payment,
      database,
    ...
    test("should suspend access after grace period expires", async ({
      page,
      auth,
      payment,
      database,
      ...
      await expect(
        page.locator('[data-cy="subscription-suspended"]'),
      ).toBeVisible();
      ...
      await page.locator('[data-cy="upload-button"]').click();
      await expect(
        page.locator('[data-cy="subscription-suspended-modal"]'),
      ).toBeVisible();

      // Can still update payment to reactivate
      await expect(
        page.locator('[data-cy="reactivate-subscription"]'),
      ).toBeVisible();
    });
  });

  test.describe("Cancellation Flow", () => {
    test("should handle full cancellation flow", async ({
      page,
      auth,
      payment,
    }) => {
      ...
      // Show survey
      await expect(
        page.locator('[data-cy="cancellation-survey"]'),
      ).toBeVisible();
    ...
    test("should offer win-back after cancellation", async ({
      page,
      auth,
      payment,
      database,
      ...
      await expect(
        stripeFrame.locator(".OrderDetails-amount"),
      ).not.toContainText("$19.00");
    });
  });
});
