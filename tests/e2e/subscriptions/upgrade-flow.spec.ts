...
test.describe('Subscription Upgrade Flow', () => {
  ...
  test.describe('Free to Pro Upgrade', () => {
    test('should successfully upgrade from free to pro plan', async ({ page, auth, payment }) => {
      // Step 1: Login as free tier user
      await auth.loginAsUser(page, 'free');
      
      // Step 2: Verify current plan status
      await expect(page.locator('[data-cy="current-plan"]')).toContainText('Free');
      ...
      await expect(proPlanCard.locator('[data-cy="plan-name"]')).toContainText('Pro');
      await expect(proPlanCard.locator('[data-cy="plan-price"]')).toContainText('$19');
      await expect(proPlanCard.locator('[data-cy="plan-credits"]')).toContainText('50 pages/month');
    ...
    test('should handle upgrade interruption gracefully', async ({ page, auth, payment }) => {
      await auth.loginAsUser(page, 'free');
      await payment.navigateToUpgrade(page, 'pro');
    ...
    test('should prevent duplicate upgrades', async ({ page, auth, payment }) => {
      await auth.loginAsUser(page, 'free');
      ...
      await expect(newPage.locator('[data-cy="upgrade-in-progress"]')).toBeVisible();
  ...
  test.describe('Pro to Business Upgrade', () => {
    test('should successfully upgrade from pro to business plan', async ({ page, auth, payment }) => {
      ...
      await expect(businessCard.locator('[data-cy="plan-name"]')).toContainText('Business');
      await expect(businessCard.locator('[data-cy="plan-price"]')).toContainText('$49');
      await expect(businessCard.locator('[data-cy="plan-credits"]')).toContainText('500 pages/month');
    ...
    test('should handle proration correctly', async ({ page, auth, payment, database }) => {
  ...
  test.describe('Upgrade Edge Cases', () => {
    test('should maintain credits immediately after upgrade', async ({ page, auth, payment, database }) => {
    ...
    test('should persist upgrade through page refresh', async ({ page, auth, payment }) => {
      await auth.loginAsUser(page, 'free');
    ...
    test('should handle network interruption during upgrade', async ({ page, auth, payment, context }) => {
      await auth.loginAsUser(page, 'free');
      await payment.navigateToUpgrade(page, 'pro');
