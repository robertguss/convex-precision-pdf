...
test.describe("Payment Failure Scenarios", () => {
  ...
  test.describe("Card Declined", () => {
    test("should handle declined card gracefully", async ({
      ...
      await expect(stripeFrame.locator(".StripeError")).toContainText(
        /declined/i,
      );
    ...
    test("should allow retry with different card", async ({
      page,
      auth,
      payment,
    }) => {
  ...
  test.describe("Insufficient Funds", () => {
    test("should handle insufficient funds error", async ({
      ...
      await payment.fillStripeCheckout(
        page,
        testCards.insufficientFunds.number,
      );
      await payment.completeStripePayment(page);
  ...
  test.describe("Expired Card", () => {
    test("should detect expired card before submission", async ({
      page,
      auth,
      payment,
    }) => {
  ...
  test.describe("3D Secure Authentication", () => {
    test("should handle 3D Secure authentication success", async ({
      page,
      auth,
      payment,
    }) => {
      ...
      const secureFrame = page.frameLocator(
        'iframe[name*="__privateStripeFrame"]',
      );
      const completeButton = secureFrame.locator(
    ...
    test("should handle 3D Secure authentication failure", async ({
      page,
      auth,
      payment,
    }) => {
  ...
  test.describe("Recovery and Retry", () => {
    test("should show retry button after failure", async ({
      page,
      auth,
      payment,
    }) => {
    ...
    test("should maintain user context through payment failure", async ({
      page,
      auth,
      payment,
    }) => {
      ...
      await expect(page.locator('[data-cy="user-name"]')).toContainText(
        userName!,
      );
